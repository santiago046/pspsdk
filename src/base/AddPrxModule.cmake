...
function(add_prx_module name)
  ...
  foreach(FILE ${ARGN})
    ...
    if ("${FILE_EXTENSION}" STREQUAL ".exp")
      ...
      get_filename_component(EXP_FILE_NAME ${FILE} NAME_WLE)
      ...
      add_custom_command(
        ...
        COMMAND "${PSPDEV}/bin/psp-build-exports" -b "${FILE}" > "${GENERATED_C_FILE}"
        DEPENDS ${FILE}
  ...
  target_link_options(${name} PRIVATE -Wl,-q,-T${PSPDEV}/psp/sdk/lib/linkfile.prx -nostartfiles)

  add_custom_command(
    TARGET ${name}
    POST_BUILD
    COMMAND "${PSPDEV}/bin/psp-fixup-imports" "$<TARGET_FILE:${name}>"
  ...
  add_custom_command(
    TARGET ${name}
    POST_BUILD
    COMMAND "${PSPDEV}/bin/psp-prxgen" "$<TARGET_FILE:${name}>" "$<TARGET_FILE:${name}>.prx"
